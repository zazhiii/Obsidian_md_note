...
1. 底层数据结构：ArrayList是`Object`**数组**，LinkedList是**双向链表**
...
# `BlockingQueue`
#[[Concurrent Program#线程池中有哪些常见阻塞队列]]

# HashMap

## HashMap底层实现原理

数据结构：使用 hash 表数据结构（散列表），即**数组 + 链表 + 红黑树**
...
- 第一次添加数据会初始化长度为 16，之后每次达到扩容阈值才会扩容
> 扩容阈值：
- **每次扩容 2 倍**
...
1. 计算索引时效率更高：可以用`hash & (n - 1)`代替`hash % n`，位运算效率更高
2. 扩容时重新计算索引效率更高：用`hash & oldCap == 0`来决定元素留在原位`i`还是`i + oldCap`，只需要判断高位的一个二进制位。

## HashMap在 1.7 多线程死循环

1.7 多个元素在一个桶时是使用「**链表**」解决的，且插入链表是使用「**头插法**」。
在**扩容**的情况下，多个线程对链表进行操作，头插法会导致链表节点指向不正确，在链表中形成环。
在后续查询的时候就会导致死循环。
...
JDK 1.8 之前：`Segment`数组 + `HashEntry`数组 + 链表
使用`Segment`分段锁数组锁住一段一段的数据，不同段的数据可以同时访问
`Segment`数组大小为 16，不可扩容，即最大支持 16 个线程并发访问

JDK 1.8 之后：`Node`数组 + CAS + synchronized + 红黑树
锁只锁一个节点
...
1. 实现线程安全的方式
2. 解决哈希冲突的方式（和 HashMap 一样）

## 与 `HashTable` 的区别?

主要是实现线程安全的方式：
`HashTable`使用一把锁（`synchronized`）锁住所有数据
